 ...
 */
let app = angular.module("DjoeInventory", ['cp.ngConfirm', 'ngDragDrop']);


app.config(['$compileProvider', ($compileProvider) => {
    $compileProvider.aHrefSanitizationWhitelist(/^\s*(https?|ftp|mailto|chrome-extension|package|nui):/);
    $compileProvider.imgSrcSanitizationWhitelist(/^\s*(https?|ftp|mailto|chrome-extension|package|file|nui|nui):/);
}]);
...
app.controller("InventoryCtrl", ['$scope', '$ngConfirm', '$q', ($scope, $ngConfirm, $q) => {
    ...
    $scope.onSlotDrop = (ev, ui, slot) => {
        ...
        if (oldSlot.item !== undefined && (oldSlot.index !== slot.index || oldSlot.item.inventoryType !== slot.type)) {
            try {
                ...
                $.post('http://djoe_inventory/inventorySwitchItem', JSON.stringify({
                        slottype    : slot.type,
                        oldtype     : oldSlot.type,
                        olditemid   : oldSlot.item.id,
                        slotindex   : slot.index,
                        invSlot     : oldSlot.item.inventorySlot
                }));
    ...
    $scope.onPocketDrop = (ev, helper) => {
        ...
        if (availableSlot !== null && availableSlot !== undefined) {
            ...
            if (oldSlot !== undefined) {
                // besoin d'être implanté pour RedM
    ...
    $scope.useItem = () => {
        if ($scope.selectedItem.quantity >= 1) {
            ...
            $.post('http://djoe_inventory/inventoryUseItem', JSON.stringify({
                id              : $scope.selectedItem.id,
                inventoryType   : $scope.selectedItem.inventoryType,
                inventorySlot   : $scope.selectedItem.inventorySlot
            }));
    ...
    $scope.equipItem = () => {
        ...
        if (item == undefined || !item.equipable)
            return;
                    ...
                    action: (scope, button) => {
                        $scope.$apply(() => {
                            ...
                            if (value <= $scope.selectedItem.quantity) {
                                ...
                                $.post('http://djoe_inventory/inventoryDropItem', JSON.stringify({
                                    inventoryType   : $scope.selectedItem.inventoryType,
                                    id              : $scope.selectedItem.id,
                                    inventorySlot   : $scope.selectedItem.inventorySlot,
                                    value           : value,
                    ...
                    action: (scope, button) => {
                        $scope.$apply(() => {
                            ...
                            if ($scope.selectedItem.quantity >= value) {
                                $scope.selectedItem.quantity -= value;
                                $scope.calculSize($scope.selectedItem.inventoryType);
                                // Besoin d'être implanté sur RedM
 ...
 */
                    ...
                    action: (scope, button) => {
                        $scope.$apply(() => {
                            ...
                            if (newSlot != null) {
                                ...
                                $.post('http://djoe_inventory/inventorySplitItem', JSON.stringify({
                                    inventoryType   : $scope.selectedItem.inventoryType,
                                    id              : $scope.selectedItem.id,
                                    newSlot         : newSlot,
                                    oldSlot         : oldSlot,
        ...
        $ngConfirm({
            ...
            buttons: {
                drop: {
                    ...
                    action: (scope, button) => {
                        $scope.$apply(() => {
                            $.post('http://djoe_inventory/inventoryChangeItemPrice', JSON.stringify({
                                inventoryType   : $scope.selectedItem.inventoryType,
                                id              : $scope.selectedItem.id,
                                inventorySlot   : $scope.selectedItem.inventorySlot,
                                value           : $scope.priceChange
                            }));
                        });
                    }
                },
    ...
    $(() => {
        ...
        window.addEventListener('message', function (event) {
            ...
            if (event.data.UIName != "Inventory") return;
           
            switch (event.data.Action) {
        
                case 'Activate':
                    console.log("Activate: " + event.data.Action)
                ...
                case 'ItemsLoaded':
                    ...
                    $scope.$apply(() => {
                        $scope.itemsPocket = data.Pocket;
                        $scope.itemsBag = data.Bag;
                        $scope.itemsDistant = data.Distant;
                        $scope.itemsOutfit = data.Outfit;
                        $scope.itemGive = data.Give;
                        $scope.loadItems();
                    });
                    break;
